...
library(tidyverse)
data_path <- "~/Documents/Site/tennis_data/"
...
```{r, echo=FALSE}
upset_df %>% 
  tidyr::spread(key = round, value = upset_points) %>% 
  select(Seed = loser_seed, F, SF, QF, R16, R32, R64, R128) %>% 
  slice(c(1, 2, 3, 5, 9)) %>% 
  mutate(Seed = c("1", "2", "3-4", "5-8", "9-16")) %>% 
  knitr::kable()
```
