...
*Note this is v2, in which the `expression` section is rewritten. To see earlier versions see [here](https://github.com/EdwinTh/EdwinTh.github.io/releases).*
...
Before we dig in an elaborate example, there is the concept of **overscope** to discuss. What would you expect the following would return?

```{r eval=FALSE}
x <- "The enclosed environment"
x_quoted <- quo(x)
eval_func <- function(q) {
  x <- "The function environment"
  rlang::eval_tidy(q)
}
eval_func(x_quoted)
```
The quosure *x_quoted* has access to two environments in which the variable *x* lives. The question is, which one prevails? The answer is:
